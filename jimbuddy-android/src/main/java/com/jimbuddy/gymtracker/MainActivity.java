...
public class MainActivity extends BridgeActivity {
    ...
    private void patchExistingFunctions(WebView webView) {
        ...
        String patchScript =
            ...
            "var sc=setInterval(function(){if(typeof sendChatNotification==='function'){clearInterval(sc);var _sc=sendChatNotification;sendChatNotification=async function(senderId,content,chatId){var cm=document.getElementById('chat-modal'),mo=cm&&cm.classList.contains('open');if(mo&&typeof activeChatFriendId!=='undefined'&&activeChatFriendId===senderId)return;var cn=_chatNotifNameCache||{},si=cn[senderId]||{},sn=si.username||'Your Gymbro';try{if(b&&typeof b.showChatNotification==='function'){b.showChatNotification(sn,content||'New message',senderId||'',chatId||'');}}catch(e){try{var _t=sn,opts={body:content||'New message',tag:'chat-'+senderId,renotify:true,icon:'/icons/icon-192.png',badge:'/icons/icon-72.png',vibrate:[100,50,200],silent:false,requireInteraction:false,timestamp:Date.now(),data:{type:'chat',friendId:senderId,chatId:chatId}};var r=await navigator.serviceWorker.getRegistration();if(r)await r.showNotification(_t,opts);else new Notification(_t,opts);}catch(e2){}}};console.log('[NotifBridge] sendChatNotification patched');}},200);" +
