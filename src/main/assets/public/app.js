...
function navigate(page) {
  ...
  requestAnimationFrame(() => {
    ...
    // This is a CRITICAL fix for low-end Android WebView GPU exhaustion (black blank screens)
    setTimeout(() => {
      document.querySelectorAll('.page:not(.active)').forEach(p => {
        p.style.display = 'none';
      });
...
function saveEditProfileDetails() {
  if (!editingProfileId) return;
  ...
  updateProfileBadge();
  renderDashboard();
  toast(`Profile "${profileName}" saved locally`);
}
        ...
        <div class="profile-header">
          ...
          <div class="profile-info">
            ...
            <div class="profile-stats">
              ${stats.sessions} sessions · ${stats.prs} PRs · 🔥 ${stats.streak} day streak
            </div>
            ${ageLine}
            <div class="profile-stats">
...
function openHydrationReminderModal() {
  ...
  if (statusEl && 'Notification' in window) {
    ...
    if (p === 'granted') {
      ...
      statusEl.textContent = '🔔 Notifications: Allowed ✅';
    } else if (p === 'denied') {
...
async function openUserProfile(userId) {
  ...
  if (!currentUser) { toast('Log in to view profiles.'); return; }
  ...
  try {
    ...
    if (!rows || rows.length === 0) { _renderUserProfileError('User profile not found.'); return; }
    const profile = rows[0];
    const { data: friendRows } = await client.from('friends').select('id').eq('user_id', currentUser.id).eq('friend_id', userId).limit(1);
    const isFriend = friendRows && friendRows.length > 0;
    let pendingStatus = null;
    if (!isFriend) {
      const { data: reqRows } = await client.from('friend_requests').select('id, sender_id, status')
...
function _renderUserProfileLoading() {
  ...
  if (body) body.innerHTML = `<div style="text-align:center;padding:40px 0;"><div style="font-size:36px;margin-bottom:12px;">⏳</div><p class="muted-text">Loading profile…</p></div>`;
}

function _renderUserProfileError(msg) {
  const body = document.getElementById('user-profile-body');
  if (body) body.innerHTML = `<div class="empty-state"><span class="empty-icon">😕</span><p>${escHtml(msg)}</p></div>`;
}
...
function _renderUserProfileContent(profile, isFriend, pendingStatus) {
  ...
  let friendBtnHtml = '';
  if (isFriend) {
    friendBtnHtml = `<div class="up-friend-status up-gymbros"><span>🤝</span> <span>Gymbros</span></div>`;
  } else if (pendingStatus === 'sent') {
    friendBtnHtml = `<div class="up-friend-status up-pending"><span>⏳</span> <span>Request Pending</span></div>`;
  } else if (pendingStatus === 'received') {
    friendBtnHtml = `<button class="btn btn-primary" onclick="handleAddFriendFromProfile('${escHtml(profile.user_id)}', '${escHtml(profile.username)}')">📨 Accept Request</button>`;
  } else {
    friendBtnHtml = `<button class="btn btn-primary" id="up-add-btn" onclick="handleAddFriendFromProfile('${escHtml(profile.user_id)}', '${escHtml(profile.username)}')">➕ Add Gymbro</button>`;
  }
  const volume = profile.total_volume >= 1000 ? (profile.total_volume / 1000).toFixed(1) + 'k' : (profile.total_volume || 0);
  ...
  if (split && split.days) {
    ...
    if (splitRows.length > 0) {
      splitHtml = `<div class="up-split-list">${splitRows.map(row => `
  ...
  const copySplitBtnHtml = hasSplit ? `<button class="btn btn-ghost btn-sm" onclick="copyUserSplit()" style="margin-top:12px;width:100%;">📋 Copy Workout Split to My Schedule</button>` : '';
  const modal = document.getElementById('user-profile-modal');
  ...
  body.innerHTML = `
    <div class="up-hero">
      ...
      <div class="up-hero-info">
        ...
        <div class="up-hero-sub">${profile.top_pr_name && profile.top_pr_name !== '—' ? `🏆 Top PR: ${escHtml(profile.top_pr_name)} ${profile.top_pr_weight ? profile.top_pr_weight + ' kg' : ''}` : ''}</div>
      </div>
      <div class="up-hero-action">${friendBtnHtml}</div>
    </div>
    <div class="up-stats-row">
      <div class="up-stat"><span class="up-stat-val">🔥 ${profile.streak || 0}</span><span class="up-stat-label">Day Streak</span></div>
      <div class="up-stat"><span class="up-stat-val">🏆 ${profile.pr_count || 0}</span><span class="up-stat-label">PRs</span></div>
      <div class="up-stat"><span class="up-stat-val">🏋️ ${volume}</span><span class="up-stat-label">Total kg</span></div>
    </div>
    <div class="up-section-header">📅 Weekly Workout Split</div>
    ${hasSplit ? splitHtml + copySplitBtnHtml : '<p class="muted-text" style="text-align:center;padding:16px 0;font-size:13px;">This user hasn\'t synced their split yet.</p>'}
  `;
}
...
async function handleAddFriendFromProfile(userId, username) {
  ...
  if (success) {
    ...
    if (heroAction) heroAction.innerHTML = `<div class="up-friend-status up-pending"><span>⏳</span> <span>Request Pending</span></div>`;
...
function copyUserSplit() {
  ...
  if (!split || !split.days || Object.keys(split.days).length === 0) { toast('No split data to copy.'); return; }
      ...
      return `
        <div class="gymbro-friend-card" id="friend-card-${friend.friend_id}">
          ...
          <div class="gymbro-friend-card-actions">
            <button class="btn btn-sm btn-ghost" onclick="openUserProfile('${friend.friend_id}')">👤</button>
